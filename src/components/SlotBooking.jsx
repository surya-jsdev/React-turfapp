  ...
  return (
    <section className='slotbooking'>
      <div className="p-6 w-6xl h-[650px] bg-[white] rounded-lg">
        ...
        <p className="text-sm text-gray-400 pb-4">Select date and time slot to book your turf</p>
        <img src={image} alt={name} className='w-xs h-40 cover rounded-md' />

        <div className="w-lg grid grid-cols-2 md:grid-cols-2 gap-4 mt-10">
          {timeslot.map((slot, index) => (
            <div
              key={index}
              className="w-[200px] p-4 border rounded-lg text-center text-black cursor-pointer  hover:bg-blue-500 hover:text-white transition"
            >
              {slot.timeslot}
            </div>
